...
if(this.arguments) var params = this.arguments[0].QueryInterface(Components.interfaces.nsIDialogParamBlock);
...
function handleRecord ( satz, ctrl ) {
    ...
    for(var i = 0; i<satzSplit.length;i++)
    {
        if(satzSplit[i] != "")
        {
            if(satzSplit[i].left(1) !="2") {
                title += "\n"  + satzSplit[i];
            }
            else
            {
                if(satzSplit[i].match(/208@/)) exit++;
                exemplare[exit] += "\n" + satzSplit[i];
            }
        }
    }
...
//application.activeWindow.clipboard = tmp_satz;
//__M("Titel mit Exemplar:\n" + tmp_satz);

    // nur Exemplare mir laufenden Bestand
...
function getMaxOccurrence ( satz ) {
    ...
    var oc;
    var ex = satz.match(/208@\/(..)/g);
    //__M("ex " +ex.length);
    if(ex)
    {
        for(var x = 0;x<ex.length;x++)
        {
            oc =  parseInt(ex[x].substr(5,2),10);
            occurrence[oc] = oc;
        }
        //__M("oc " +occurrence[occurrence.length-1]);
        
        return occurrence[occurrence.length-1];
    }
    else
    {
        return (0);
...
//__M(line);
    ...
    idx = -1
    while (++idx < ctrl.length) {
        ctrl[idx].val[exit] = ctrl[idx].val[exit].replace(/&amp;/g,"&");
        ctrl[idx].val[exit] = ctrl[idx].val[exit].replace(/&lt;/g,"<");
        ctrl[idx].val[exit] = ctrl[idx].val[exit].replace(/&gt;/g,">");
        line += '"' + ctrl[idx].val[exit].replace(/\u0022/g,"'") + '"\t';
    }
    line = line.replace(/;$/,"");
...
function createResult ( satz, ctrl,exit ) {
	var tag, suche, regex, group, text, idx, groupselect;
		...
		if(group)
		{
			...
			if (ctrl[idx].tag == "031N" || ctrl[idx].tag == "231@")
			{
				...
				if(satz.indexOf(String.fromCharCode(charCode)+"0") != -1)
				{
					...
					ctrl[idx].val[exit] = tempArray.join(strTrennzeichen);
				} 
				// normale prozedur
				else 
				{
